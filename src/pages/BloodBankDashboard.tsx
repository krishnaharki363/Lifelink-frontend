...
const NAV: { key: Tab; label: string; icon: React.ElementType }[] = [
  ...
  { key: 'donors',     label: 'Blood Donations',  icon: Users },
  { key: 'analytics',  label: 'Analytics',       icon: BarChart3 },
];

/* ── Mock data ─────────────────────────────────────────────────── */
const INITIAL_INVENTORY = [
  { type: 'A+',  units: 32, capacity: 100, expiringSoon: 3 },
  { type: 'A-',  units: 8,  capacity: 40, expiringSoon: 0 },
  { type: 'B+',  units: 24, capacity: 80, expiringSoon: 1 },
  { type: 'B-',  units: 6,  capacity: 30, expiringSoon: 0 },
  { type: 'O+',  units: 45, capacity: 120, expiringSoon: 5 },
  { type: 'O-',  units: 12, capacity: 50, expiringSoon: 1 },
  { type: 'AB+', units: 15, capacity: 40, expiringSoon: 2 },
  { type: 'AB-', units: 4,  capacity: 20, expiringSoon: 0 },
];
...
export const BloodBankDashboard: React.FC = () => {
  ...
  const name = user?.name || 'Red Cross Blood Bank';
  ...
  useEffect(() => {
    localStorage.setItem('bloodbank_inventory', JSON.stringify(inventory));
  }, [inventory]);
  ...
  return (
    <div className="dash-layout">
      ...
      <aside className="sidebar">
        ...
        <p className="nav-section-title">Blood Bank Portal</p>
        ...
        <div style={{ borderTop: '1px solid var(--border)', paddingTop: '1rem' }}>
          <div style={{ padding: '0.6rem 0.85rem', marginBottom: '0.5rem' }}>
            ...
            <p style={{ margin: 0, fontSize: '0.72rem', color: 'var(--gray-400)' }}>Blood Bank Account</p>
          </div>
      ...
      <main className="dash-main">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: '1.25rem' }}>
              ...
              <div className="card">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
                  <h3 style={{ margin: 0 }}>Blood Bank Inventory</h3>
        ...
        {tab === 'analytics' && (
          <div className="animate-fade-up">
            <div className="grid-4" style={{ marginBottom: '1.5rem' }}>
              {[
                { label: 'Requests This Month',  value: '42',   icon: ClipboardList, color: 'var(--red-600)' },
                { label: 'Fulfillment Rate',      value: '92%',  icon: CheckCircle2,  color: 'var(--success)' },
                { label: 'Avg Fulfillment Time',  value: '25min',icon: Clock,         color: 'var(--info)' },
                { label: 'Donor Response Rate',   value: '81%',  icon: TrendingUp,    color: 'var(--warning)' },
              ].map(s => {
                const Icon = s.icon;
                return (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
              ...
              <div className="card">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.75rem', background: 'var(--success-bg)', borderRadius: 'var(--radius-md)', marginTop: '0.5rem' }}>
                  ...
                  <span style={{ fontSize: '0.85rem', color: 'var(--success)', fontWeight: 600 }}>Blood bank stock health: 92%</span>
                </div>
              </div>
            </div>
          </div>
